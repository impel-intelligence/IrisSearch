...
struct SlotMapTest {
    ...
    ///
    /// The decoder bounds `slotCount` against this, so a test that passes a literal here is
    /// lying about the file and will trip the check for the wrong reason.
    ///
    ...
    /// A valid encoded header plus the file length that matches it.
    ///
    /// - Authored by: Claude Opus 5 (Anthropic)
    static func validHeaderBytes(
        ...
        return (header.encoded(), fileLength(forSlots: Int(slots)))
    }
    ...
    @Test func testCreateNew() async throws {
        ...
        let map = SlotMap(entries: entries)
    ...
    @Test func testAppendEntries() async throws {
        ...
        let map = SlotMap(entries: [])

        map.append(contentsOf: entries)
    ...
    @Test func testMultiByteFieldsAreLittleEndianOnDisk() async throws {
        let (bytes, _) = Self.validHeaderBytes(slots: 0x0102_0304_0506_0708, generation: 0)
    ...
    @Test func testHeaderRoundTrip() async throws {
        ...
        let fileSize = SlotMap.Header.byteCount + (MemoryLayout<UInt64>.size * Int(slotCount))
        let header = SlotMap.Header(slotCount: slotCount, generation: 3, flags: .cleanShutdown)
        let data = header.encoded()
        let decoded = try SlotMap.Header(bytes: data, fileLength: fileSize)
    ...
    @Test func testReEncodingADecodedHeaderReproducesTheSameBytes() async throws {
        let (bytes, length) = Self.validHeaderBytes()
        let decoded = try SlotMap.Header(bytes: bytes, fileLength: length)
    ...
    @Test func testEmptyHeaderRoundTrip() async throws {
        let header = SlotMap.Header(slotCount: 0, generation: 0, flags: .empty)
        let decoded = try SlotMap.Header(bytes: header.encoded(), fileLength: Self.fileLength(forSlots: 0))
    ...
    @Test func testRejectsBufferShorterThanHeader() async throws {
        ...
        #expect {
            _ = try SlotMap.Header(bytes: short, fileLength: 64)
        } throws: { error in
    ...
    @Test func testRejectsForeignMagicWithoutClaimingCorruption() async throws {
        ...
        var (bytes, length) = Self.validHeaderBytes()
        bytes.replaceSubrange(0..<4, with: Array("SQLi".utf8))

        #expect {
            _ = try SlotMap.Header(bytes: bytes, fileLength: length)
        } throws: { error in
    ...
    @Test func testRejectsAnyFlippedBitInTheChecksummedRange() async throws {
        ...
        let (original, length) = Self.validHeaderBytes()
        ...
        for index in 4..<28 {   // magic is skipped: it fails the earlier, more specific gate
            ...
            #expect {
                _ = try SlotMap.Header(bytes: corrupted, fileLength: length)
            } throws: { error in
                switch error {
    ...
    @Test func testRejectsSlotCountLargerThanTheFileCanHold() async throws {
        ...
        #expect {
            _ = try SlotMap.Header(bytes: header.encoded(), fileLength: Self.fileLength(forSlots: 3))
        } throws: { error in
    ...
    @Test func testAcceptsSlotCountExactlyFillingTheFile() async throws {
        ...
        let decoded = try SlotMap.Header(bytes: header.encoded(), fileLength: Self.fileLength(forSlots: 3))
    ...
    @Test func testDecodingArbitraryBytesNeverTraps() async throws {
        ...
        for _ in 0..<200 {
            ...
            _ = try? SlotMap.Header(bytes: noise, fileLength: 64)
        }
    }
    ...
    @Test func testEntriesAreWrittenAfterTheHeaderInOrder() async throws {
        let map = SlotMap(entries: [10, 20, 30])
    ...
    @Test func testHeaderSlotCountMatchesEntriesAfterEveryMutation() async throws {
        ...
        let map = SlotMap(entries: [1, 2, 3])
        #expect(map.header.slotCount == map.count)

        map.append(contentsOf: [4, 5, 6, 7])
        #expect(map.header.slotCount == map.count)
        ...
        let decoded = try SlotMap.Header(bytes: encoded, fileLength: encoded.count)
    ...
    @Test func testAppendReturnsTheRangeItWrote() async throws {
        ...
        let first = map.append(contentsOf: [1, 2, 3])
        #expect(first == 0..<3)

        let second = map.append(contentsOf: [4, 5])
    ...
    @Test func testAppendOfNothingChangesNothing() async throws {
        let map = SlotMap(entries: [1, 2, 3])
        let range = map.append(contentsOf: [])
    ...
    @Test func testTombstoneMarksTheFirstDocumentsRange() async throws {
        ...
        let map = SlotMap(entries: [1, 2, 3, 4, 5])
    ...
    @Test func testDeadCountIsRecoveredFromEntriesOnLoad() async throws {
        let map = SlotMap(entries: [1, SlotMap.tombstoneValue, 3, SlotMap.tombstoneValue])
    ...
    static func roundTrip(_ map: SlotMap) throws -> SlotMap {
        let bytes = map.encoded()
        return try SlotMap(bytes: bytes, fileSize: bytes.count)
    }

    @Test func testEmptyMapRoundTrip() async throws {
        let decoded = try Self.roundTrip(SlotMap(entries: []))
    ...
    @Test func testEntriesSurviveRoundTripInOrder() async throws {
        ...
        let decoded = try Self.roundTrip(SlotMap(entries: entries))
    ...
    @Test func testRoundTripIsAFixedPointOnBytes() async throws {
        ...
        let map = SlotMap(entries: [1, 2, 3, SlotMap.tombstoneValue, 5])
        let first = map.encoded()
        let second = try SlotMap(bytes: first, fileSize: first.count).encoded()
    ...
    @Test func testDecodePreservesHeaderState() async throws {
        ...
        let decoded = try SlotMap(bytes: bytes, fileSize: bytes.count)
    ...
    @Test func testDecodedMapCanBeAppendedToAndReEncoded() async throws {
        ...
        let original = SlotMap(entries: [1, 2, 3])
        let decoded = try Self.roundTrip(original)

        let range = decoded.append(contentsOf: [4, 5])
    ...
    @Test func testDecodeRejectsFewerSlotBytesThanTheHeaderClaims() async throws {
        ...
        #expect {
            _ = try SlotMap(bytes: bytes, fileSize: bytes.count)
        } throws: { error in
            switch error {
    ...
    @Test func testDecodePropagatesHeaderErrors() async throws {
        var bytes = SlotMap(entries: [1, 2, 3]).encoded()
        bytes.replaceSubrange(0..<4, with: Array("SQLi".utf8))
